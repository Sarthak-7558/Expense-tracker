...
const Transaction = () => {
  ...
  // existingtransactions.map((data,i)=>{
  //   console.log(data,i);
  // })
       ...
       <tbody>
        {transaction.map((tx,index)=>(
           <tr key={index}>
            ...
            <td className={tx.type=="income" ? 'income':'expense'}>{tx.amount }</td>
            <td>{tx.date}</td>
            <td>{tx.type}</td>
            <td>
              <div className='action-buttons'>
                <button className='edit-button' onClick={()=>handleEdit(index)}>🖊️ Edit</button>
                <button className='delete-button' onClick={()=>handleDelete(index)}>🗑️ Delete</button>
              </div>
            </td>
